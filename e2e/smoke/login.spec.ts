...
test.describe("Tests for the Login page", () => {
  test.beforeEach(async ({ page }) => {
    homePage = new HomePage(page);
    logInPage = new LogInPage(page);
      await logInPage.goto("https://login.funda.nl/account/login");
      await logInPage.acceptCookies();
    });

    test("Verify that a user can log in", async ({
      page,
    }) => {
        ...
        await expect(logInPage.emailField).toHaveAttribute("aria-invalid", "false");
